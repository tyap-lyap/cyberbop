...
public class LeftArmPartRenderer extends CyborgPartRenderer {
	...
	public void render(PlayerEntityModel<?> contextModel, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, LivingEntity entity) {
		this.model.get().getLeftArm().copyTransform(contextModel.leftArm);
		...
		this.model.get().getLeftArm().render(matrices, vertexConsumer, light, OverlayTexture.DEFAULT_UV);
	}
	...
	public void renderAssembler(AssemblerBlockEntity assembler, BlockState state, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
		matrices.push();
		this.model.get().getLeftArm().resetTransform();
		this.model.get().getLeftArm().translate(new Vector3f(5, -22, 0));
