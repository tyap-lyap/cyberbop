...
public class RightLegPartRenderer extends CyborgPartRenderer {
	...
	public void render(PlayerEntityModel<?> contextModel, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, LivingEntity entity) {
		this.model.get().getRightLeg().copyTransform(contextModel.rightLeg);
		...
		this.model.get().getRightLeg().render(matrices, vertexConsumer, light, OverlayTexture.DEFAULT_UV);
	}
	...
	public void renderAssembler(AssemblerBlockEntity assembler, BlockState state, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
		matrices.push();
		this.model.get().getRightLeg().resetTransform();
		this.model.get().getRightLeg().translate(new Vector3f(-1.9F, -12, 0));
