...
public class RightArmPartRenderer extends CyborgPartRenderer {
	...
	public void render(PlayerEntityModel<?> contextModel, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, LivingEntity entity) {
		this.model.get().getRightArm().copyTransform(contextModel.rightArm);
		...
		this.model.get().getRightArm().render(matrices, vertexConsumer, light, OverlayTexture.DEFAULT_UV);
	}
	...
	public void renderAssembler(AssemblerBlockEntity assembler, BlockState state, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
		matrices.push();
		this.model.get().getRightArm().resetTransform();
		this.model.get().getRightArm().translate(new Vector3f(-5, -22, 0));
